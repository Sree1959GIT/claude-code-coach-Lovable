...
export function ImportLogsPanel() {
  ...
  return (
    <div className="mt-4 border border-border bg-background p-5">
      <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Import logs</p>

      {runs.isLoading && (
        <p className="mt-3 font-mono text-xs text-muted-foreground">Loading…</p>
      )}
      {runs.data?.length === 0 && (
        <p className="mt-3 font-mono text-xs text-muted-foreground">No imports recorded yet.</p>
      )}
                ...
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      ...
                      className="border border-border px-2 py-1 text-xs font-bold uppercase tracking-widest hover:border-primary"
                    >
        ...
        <div className="mt-4 border-t border-border pt-4">
          <div className="flex items-center gap-3">
            {(["problems", "all"] as const).map((f) => (
              ...
              >
                {f === "problems" ? "Problems only" : "All rows"}
              </button>
            ))}
          </div>

          {items.isLoading && (
            <p className="mt-3 font-mono text-xs text-muted-foreground">Loading rows…</p>
          )}
                ...
                <tbody>
                  {rows.map((i) => (
                    <tr key={i.id} className="border-t border-border align-top">
                      ...
                      <td className={`px-3 py-2 uppercase tracking-widest text-xs ${STATUS_CLASS[i.status] ?? ""}`}>
                        {i.status}
                      </td>
