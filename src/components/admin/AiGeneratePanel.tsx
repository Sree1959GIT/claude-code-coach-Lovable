  ...
  return (
    <div className="mt-4 border border-border bg-background p-5">
      <div className="flex flex-wrap items-end gap-4">
        <label className="flex flex-col gap-1 font-mono text-xs uppercase tracking-widest text-muted-foreground">
          ...
          <select
            ...
            className="border border-border bg-muted/20 px-2 py-1.5 font-mono text-xs normal-case tracking-normal text-foreground outline-none focus:border-primary"
          >
            {domains.map((d) => (
        ...
        <label className="flex flex-col gap-1 font-mono text-xs uppercase tracking-widest text-muted-foreground">
          ...
          <input
            ...
            className="w-20 border border-border bg-muted/20 px-2 py-1.5 font-mono text-xs text-foreground outline-none focus:border-primary"
          />
        </label>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          {mutation.isPending ? "Generating…" : "Preview drafts"}
        </button>
        <button
          type="button"
          ...
          <div className="mt-4 space-y-4">
            {result.drafts.map((d, i) => (
              <article key={i} className="border border-border p-4">
                ...
                {d.scenario && (
                  <p className="mt-3 font-mono text-xs leading-relaxed text-muted-foreground">{d.scenario}</p>
                )}
                <p className="mt-2 font-mono text-[12px] font-bold leading-relaxed">{d.stem}</p>
                <ul className="mt-3 space-y-2">
                  {d.options.map((o) => (
                    <li key={o.label} className="font-mono text-xs leading-relaxed">
                      <span className={o.isCorrect ? "font-bold text-primary" : "text-muted-foreground"}>
                        {o.label}.
                      </span>{" "}
                      <span className={o.isCorrect ? "font-bold" : ""}>{o.text}</span>
                      {o.explanation && (
                        <span className="block pl-4 text-xs text-muted-foreground">{o.explanation}</span>
                      )}
                    </li>
                  ))}
                </ul>
